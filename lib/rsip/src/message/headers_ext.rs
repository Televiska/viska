...
impl HeadersExt for SipMessage {
    ...
    fn contact_headers(&self) -> Vec<&headers::Contact> {
        match self {
            Self::Request(request) => request.contact_headers(),
            Self::Response(response) => response.contact_headers(),
        }
    }
    fn user_agent_header(&self) -> Result<&headers::UserAgent, Error> {
        match self {
...
impl HeadersExt for crate::Headers {
    fn to_header(&self) -> Result<&headers::To, Error> {
        header!(
            self.iter(),
            Header::To,
    ...
    fn contact_headers(&self) -> Vec<&headers::Contact> {
        all_headers!(self.iter(), Header::Contact)
    }
