...
use sip_server::{core::impls::UasProcessor, ReqProcessor, SipBuilder, SipManager, CoreProcessor};
use std::sync::Arc;

async fn setup() -> (
    UasProcessor<RegistrarSnitch, ReqProcessorPanic>,
    Arc<SipManager>,
) {
    ...
    let processor: UasProcessor<RegistrarSnitch, ReqProcessorPanic> =
        UasProcessor::new(Arc::downgrade(&sip_manager));
